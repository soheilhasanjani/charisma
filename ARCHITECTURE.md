...
flowchart TB
    ...
    subgraph state [استورها - یکی برای هر بخش زنده]
      ...
      History["HistoryStore - ring buffer"]
...
- **چه چیزی می‌تواند بی‌مرز رشد کند؟** هیچ‌چیز. تاریخچهٔ هر نماد ring buffer با ظرفیت ثابت است (۱۲۰ قیمت، ۲۰ معامله) و قدیمی‌ترین را می‌اندازد؛ وضعیت یک خانه است؛ بنر آخرین معامله یک خانه با throttle خوانایی است. یک تب که تمام شب باز بماند حافظه نمی‌خورد.
- **اگر فریم از بودجه رد شد؟** scheduler به flush یک فریم در میان یا یکی از هر سه می‌افتد، به جای اینکه پیوسته عقب بیفتد.
- **مرتب‌سازی روی ستون زنده؟** با ۵۰۰۰ ردیف و مرتب‌سازی روی امتیاز ریسک، ردیف‌ها مدام از هم سبقت می‌گیرند و جدول غیرقابل استفاده می‌شود؛ راه‌حل «مرتب‌سازی سریع‌تر» نیست. ترتیب با throttle بازمحاسبه می‌شود و تا وقتی نشانگر روی جدول است یا اسکرول در جریان است ثابت نگه داشته می‌شود.
...
| بازمحاسبهٔ ترتیب       | مرتب‌سازی ۵۰۰۰ عضوی روی هر فریم گران است                    | throttle + قفل ترتیب هنگام hover/scroll + مقایسهٔ آرایه قبل از notify | ساختار مرتب افزایشی (heap) به جای sort کامل                         |
...
- **heartbeat وجود ندارد.** mock پیام‌های ناشناس را نادیده می‌گیرد، پس `ping`/`pong` و اندازه‌گیری RTT ممکن نیست و staleness تنها سیگنال زنده‌بودن است. جای اتصالش `touchWatchdog()` در `socket-client.ts` است.
